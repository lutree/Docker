...
su - steam -c "${STEAMCMDDIR}/steamcmd.sh +force_install_dir \"${STEAMAPPDIR}\" \
                                    +login anonymous \
...
#  su - steam -c "rm -rf /home/steam/Zomboid/Server/*"
#  su - steam -c "git clone https://github.com/lutree/pzserver_config.git /home/steam/Zomboid/Server"
#else
#  cd /home/steam/Zomboid/Server
#  su - steam -c "git fetch --all"
#  su - steam -c "git reset --hard origin/master"
#  su - steam -c "git pull origin/master"
#fi

su - steam -c "export LD_LIBRARY_PATH=\"${STEAMAPPDIR}/jre64/lib:${LD_LIBRARY_PATH}\" && \
    export LC_ALL=\"ko_KR.EUC-KR\" && \
    export LANG=\"ko_KR.EUC-KR\" && \
    export LANGUAGE=\"ko_KR.EUC-KR\" && \
    cd ${STEAMAPPDIR} && \
    pwd && \
